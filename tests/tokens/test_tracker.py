from llmify.tokens import TokenTracker, TokenUsageEntry, UsageSummary
...
class TestAddAcceptedInputs:
    def test_accepts_raw_usage(self) -> None:
        ...
        tracker.add(make_usage(prompt=10, completion=5), model="gpt-4o")
    ...
    def test_accepts_chat_invoke_completion(self) -> None:
        ...
        tracker.add(completion, model="gpt-4o")
    ...
    def test_accepts_stream_end(self) -> None:
        ...
        tracker.add(end, model="claude-sonnet-4-20250514")

        assert tracker.entries[0].total_tokens == 10
        assert tracker.entries[0].model == "claude-sonnet-4-20250514"


class TestAddNoOps:
    def test_none_usage_is_ignored(self) -> None:
        tracker = TokenTracker()
        tracker.add(None, model="gpt-4o")

        assert tracker.entry_count == 0

    def test_completion_without_usage_is_ignored(self) -> None:
        tracker = TokenTracker()
        tracker.add(ChatInvokeCompletion(completion="hi", usage=None), model="gpt-4o")

        assert tracker.entry_count == 0

    def test_stream_end_without_usage_is_ignored(self) -> None:
        tracker = TokenTracker()
        tracker.add(StreamEnd(usage=None), model="gpt-4o")
...
class TestTagging:
    def test_entry_is_tagged_with_model(self) -> None:
        tracker = TokenTracker()
        tracker.add(make_usage(), model="gpt-4o")
        tracker.add(make_usage(), model="claude-sonnet-4-20250514")
    ...
    def test_cached_tokens_preserved_on_entry(self) -> None:
        tracker = TokenTracker()
        tracker.add(make_usage(cached=4), model="gpt-4o")
...
class TestSummary:
    ...
    def test_aggregates_across_entries_and_models(self) -> None:
        tracker = TokenTracker()
        tracker.add(make_usage(prompt=10, completion=5, cached=2), model="gpt-4o")
        tracker.add(make_usage(prompt=20, completion=8, cached=3), model="claude")
    ...
    def test_treats_missing_cached_tokens_as_zero(self) -> None:
        tracker = TokenTracker()
        tracker.add(make_usage(cached=None), model="gpt-4o")
        tracker.add(make_usage(cached=4), model="gpt-4o")
...
class TestStateManagement:
    def test_total_tokens_property_matches_summary(self) -> None:
        tracker = TokenTracker()
        tracker.add(make_usage(prompt=10, completion=5), model="gpt-4o")
        tracker.add(make_usage(prompt=1, completion=1), model="gpt-4o")
